...
Ext.define('MyDesktop.Landlord.Landlord', {
    ...
    lord: function(region, landLordPanel){
        var sentParam = {userId: 0};
        
        layer = new OpenLayers.Layer.Vector("لایه قطعات یک زمین", {
        ...
        var landModel = Ext.define('LandModel', {
            extend: 'Ext.data.Model',
            fields: [
                {name: 'gid', type: 'int'},
        ...
        landLordPanel.getGridPanel().getSelectionModel().on('selectionchange', function(sm, selectedRecord) {
    ...
    map: function (region){
        map = new OpenLayers.Map('Our map');
    ...
    allData: function(rootThis){
        ...
        var landLordPanel = new rootThis.landLord("center");
        var lordPanel = new rootThis.lord("south", landLordPanel);
        var mapPanel = new rootThis.map("west");
        
        
        this.panel = Ext.create('Ext.Panel', {
            layout: 'border',
            items: [ landLordPanel.getGridPanel(), lordPanel.getGridPanel(), mapPanel.getPanel()]
        });
    },

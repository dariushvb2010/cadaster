...
class RDbAuthManager extends CDbAuthManager
{
	...
	*/
		...
		if( $sort===true )
		{
			if( $type===null && $userId===null )
			{
				$sql = "SELECT name,t1.type,description,t1.bizrule,t1.data,weight
					FROM {$itemTable} t1
					LEFT JOIN {$rightsTable} t2 ON name=itemname
			...
			else if( $type===null )
			{
				$sql = "SELECT name,t1.type,description,t1.bizrule,t1.data,weight
					FROM {$itemTable} t1
					LEFT JOIN {$assignmentTable} t2 ON name=t2.itemname
					LEFT JOIN {$rightsTable} t3 ON name=t3.itemname
					WHERE userid=:userid
					ORDER BY t1.type DESC, weight ASC";
				$command=$this->db->createCommand($sql);
	...
	public function getAssignmentsByItemName($name)
	{
		...
		$command->from($this->assignmentTable)->where("itemname=:itemname");
	...
	*/
	public function updateItemWeight($result)
	{
		...
		{
			$sql = "SELECT COUNT(*) FROM {$rightsTable}
			...
			else
			{
				...
				{
					$sql = "INSERT INTO {$rightsTable} (itemname, type, weight)
